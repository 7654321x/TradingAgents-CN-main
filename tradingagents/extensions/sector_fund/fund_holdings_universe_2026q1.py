...
def validate_fund_holdings_seed(data: dict[str, Any]) -> None:
    if not {"schema_version","holding_report_period_end","holding_report_published_date","funds"} <= set(data):
        raise ValueError("invalid fund holdings seed header")
    funds = data["funds"]
    if len(funds) != 3 or {str(x.get("fund_code")) for x in funds} != {"020671","017811","025500"}: raise ValueError("expected three known funds")
    for fund in funds:
        if len(fund.get("holdings", [])) != 10: raise ValueError(f"fund {fund.get('fund_code')} must have ten holdings")
        for holding in fund["holdings"]:
            if not str(holding.get("symbol", "")).endswith((".SS", ".SZ")): raise ValueError(f"invalid A-share symbol: {holding.get('symbol')}")
...
def build_stock_universe(seed: dict[str, Any]) -> tuple[dict[str, Any], ...]:
    ...
    for fund in seed["funds"]:
        for holding in fund["holdings"]:
            ...
            item = instruments.setdefault(symbol, {"symbol":symbol,"local_code":holding["local_code"],"name":holding["name"],"instrument_type":"stock","fund_membership":[]})
            item["fund_membership"].append({"fund_code":fund["fund_code"],"rank":holding["rank"],"weight_pct":holding["weight_pct"]})
...
def build_proxy_universe(seed: dict[str, Any]) -> tuple[dict[str, Any], ...]:
    ...
    return tuple(sorted(result, key=lambda x:x["symbol"]))
