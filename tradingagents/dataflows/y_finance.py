...
def _get_yfinance_daily_frame(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Return raw daily Yahoo bars for MarketDataService provider mode."""
    datetime.strptime(start_date, "%Y-%m-%d")
    end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    canonical = normalize_symbol(symbol)
    end_inclusive = (end_dt + relativedelta(days=1)).strftime("%Y-%m-%d")
    data = yf_retry(
        lambda: yf.Ticker(canonical).history(start=start_date, end=end_inclusive)
    )
    if data.empty:
...
def _format_stock_data(
    data: pd.DataFrame, symbol: str, start_date: str, end_date: str
) -> str:
    canonical = normalize_symbol(symbol)
...
def get_YFin_data_online(
    ...
    """Direct Yahoo adapter retained for explicit provider use."""
    data = _get_yfinance_daily_frame(symbol, start_date, end_date)
    return _format_stock_data(data, symbol, start_date, end_date)
...
def get_database_stock_data(
    ...
    """Production stock-data entry point governed by ``market_data.mode``."""
    ...
    config = get_config().get("market_data", {})
    mode = config.get("mode", "database_only")
    with Session(get_engine()) as session:
        service = MarketDataService(
            session,
            mode=mode,
            provider=_get_yfinance_daily_frame,
            include_unfinished_daily_bar=config.get(
                "include_unfinished_daily_bar", False
            ),
            persist_provider_results=config.get("persist_provider_results", False),
        )
        result = service.daily(symbol, start_date, end_date)

    logger.info(
        "market_data_mode=%s market_data_source=%s market_data_refreshed=%s "
        "provider_call_count=%s returned_rows=%s",
        mode,
        result.source,
        result.refreshed,
    ...
    if result.status != SUCCESS:
        return (
            f"{result.status}: symbol={symbol}, requested_start={start_date}, "
            f"requested_end={end_date}, mode={mode}, detail={result.message}"
        )
    return _format_stock_data(result.data, symbol, start_date, end_date)
