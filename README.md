...
默认关注方向包括半导体、存储芯片、芯片概念、科创芯片、半导体设备、先进封装、PCB、消费电子和 AI 芯片；默认基金为 `020671 易方达科创芯片ETF联接C` 与 `025500 东方阿尔法科技智选混合C`。当前科技仓位默认 20%，目标仓位 25%-30%，不建议超过 35%。

运行 mock 模式：

```bash
python main.py --mode sector_fund --config config/personal_semiconductor.yaml --mock
```

或使用 CLI：

```bash
python -m cli.main analyze-sector-fund --config config/personal_semiconductor.yaml --mock
```

运行真实网页 raw_text 采集模式：

```bash
python main.py --mode sector_fund --config config/personal_semiconductor.yaml --real-data
```
...
python main.py --mode sector_fund --config config/personal_semiconductor.yaml --real-data --firecrawl
```

如果没有 `FIRECRAWL_API_KEY`，或东方财富、同花顺、天天基金、巨潮资讯等公开网页抓取失败，程序不会崩溃，会保留 mock 结构化数据并在数据源状态中标记失败，报告字段缺失时显示“未获取到”。

真实数据模式会在报告“数据源状态”中追加字段来源：

- `real_data`：从公开网页 raw_text 成功解析出的结构化字段
- `firecrawl_raw`：从 Firecrawl 返回的 raw_text 成功解析出的结构化字段
- `mock_fallback`：真实字段未解析成功，继续使用 mock 兜底字段
- `missing`：对应网页或字段缺失
- `insufficient_history`：字段需要本地历史数据计算，但历史样本不足

ETF 与龙头股增强：

- 真实模式会按 `personal_semiconductor.yaml` 中的 ETF 和 `watch_stocks` 生成东方财富/同花顺 raw_text 页面。
- ETF 支持解析价格、涨跌幅、成交额、换手率、溢价率、近5日涨幅。
- ETF MA5/MA10/MA20 不强依赖网页字段，会把真实价格保存到 `data/sector_fund_history.json`；历史足够后自动计算均线和回踩/跌破状态。
- 龙头股支持解析涨跌幅、成交额、换手率、主力净流入、开高低收、昨收，并据此判断长上影和冲高回落。
- 龙头股真实 close 解析成功后会写入同一个本地历史文件；历史满 5/10 条后计算个股 MA5/MA10 和跌破状态，历史不足时标记 `insufficient_history`。
- 龙虎榜 raw_text 支持解析是否上榜、机构净买入、游资净买入、买卖前五合计、净买额、上榜原因和情绪标签。
- 公告 raw_text 支持识别业绩预增、订单增长、客户验证、减持、业绩预亏、风险提示等关键词，并在“公告与新闻风险”中区分利好公告和利空公告。
- 报告“数据源状态”会显示【数据可信度】：真实解析字段、mock兜底字段、缺失字段、真实覆盖率和数据质量等级。
