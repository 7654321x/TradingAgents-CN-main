...
@app.command(
    name="analyze-sector-fund",
    help="个人半导体/存储板块与基金持仓分析 | Personal semiconductor sector and fund analysis"
)
def analyze_sector_fund(
    ...
    mock_data: bool = typer.Option(
        True,
        "--mock/--real-data",
        help="使用mock结构化数据，或尝试真实网页raw_text采集",
    ),
    firecrawl: bool = typer.Option(
        False,
        "--firecrawl/--no-firecrawl",
        help="真实数据模式下是否优先使用可选Firecrawl采集网页",
    ),
    ...
    Generate a personal semiconductor/storage sector and fund holding report.
    """
    ...
    console.print("[bold green]✅ 个人半导体/存储板块报告已生成[/bold green]")
    console.print(f"半导体评分: [cyan]{score['semiconductor_score']}[/cyan]")
    console.print(f"存储芯片评分: [cyan]{score['storage_score']}[/cyan]")
...
def help_chinese():
    ...
    logger.info(f"5. [cyan]python -m cli.main analyze-sector-fund --mock[/cyan] # 个人半导体/存储报告")
    ...
    if len(sys.argv) == 1:
        ...
        try:
            ...
            if e.code == 2 and len(sys.argv) > 1:
                ...
                available_commands = ['analyze', 'analyze-sector-fund', 'config', 'version', 'data-config', 'examples', 'test', 'help']
